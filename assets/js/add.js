document.addEventListener('DOMContentLoaded', () => {
    ...
    const IMAGE_UPLOAD_PATH = '/uploads/images_tmp/';
    ...
    const showPreview = (preview, placeholder, src) => {
        preview.src = src;
        preview.classList.remove('hidden');
        placeholder?.classList.add('hidden');
    };

    const hidePreview = (preview, placeholder) => {
    ...
    const initImageItem = (item, isNew = false) => {
        ...
        // Correction automatique du src pour images temporaires
        if (hiddenInput.value && !hiddenInput.value.startsWith('/uploads')) {
            preview.src = IMAGE_UPLOAD_PATH + hiddenInput.value;
            preview.classList.remove('hidden');
            placeholder?.classList.add('hidden');
        }
        ...
        const upload = async (file, replace = false) => {
            ...
            try {
                const res = await fetch(IMAGE_ROUTES.upload, { method: 'POST', body: formData });
                ...
                if (replace && hiddenInput.value) {
                    await fetch(IMAGE_ROUTES.delete, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: new URLSearchParams({ filename: hiddenInput.value })
                    });
                }

                hiddenInput.value = image.filename;
                showPreview(preview, placeholder, IMAGE_UPLOAD_PATH + image.filename);
                updateUI();
        ...
        input?.addEventListener('change', () => {
            if (input.files?.[0]) upload(input.files[0], !!hiddenInput.value);
        });
        ...
        removeBtn?.addEventListener('click', () => {
            ...
            if (removedInput && filename) removedInput.value = filename;
            item.classList.add('opacity-30', 'pointer-events-none');
        });

        if (!preview.src) hidePreview(preview, placeholder);
        updateUI();
    };
    ...
    const addImage = () => {
        ...
        div.className = 'media-item media-image relative w-full md:w-40 md:flex-shrink-0 md:snap-start';
